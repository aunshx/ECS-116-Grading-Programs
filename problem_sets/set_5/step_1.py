...
main_zip_path = 'PS5.zip'  # Update this to the correct path
...
def extract_files(zip_path, extract_to):
    """ Recursively extract .csv files from zip files """
...
def process_student_files():
    """ Process each student's zip file within the main PS5.zip """
    with zipfile.ZipFile(main_zip_path, 'r') as main_zip:
        file_count = len(main_zip.infolist())
        print(f"Total files in PS5.zip: {file_count}")
