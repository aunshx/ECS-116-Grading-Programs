...
-- (Try to build this by yourself, before looking at how I do it!)
...
-- select mce.country_or_area from m49_codes_expanded mce where sub_region_name = 'Africa';
select distinct sub_region_name from m49_codes_expanded where region_name = 'Africa' order by sub_region_name;
select distinct intermediate_region_name from m49_codes_expanded where region_name = 'Africa' order by intermediate_region_name;
select distinct intermediate_region_name, country_or_area from m49_codes_expanded where region_name = 'Africa' order by intermediate_region_name, country_or_area;
...
select distinct intermediate_region_name from m49_codes_expanded where region_name = 'Africa' and intermediate_region_name = 'NULL' order by intermediate_region_name;
select distinct intermediate_region_name, country_or_area 
...
select m49_code , country_or_area 
from m49_codes_expanded 
where region_name = 'Africa' and intermediate_region_name = ''
order by country_or_area;
...
select country_or_area, intermediate_region_name, sub_region_name, m49_code, region_name 
from m49_codes_expanded 
where region_name = 'Africa'
...
select country_or_area, intermediate_region_name, intermediate_region_code, sub_region_name, m49_code, region_name  
from m49_codes_expanded mce 
where region_name = 'Africa';
...
select a.area_code_m49, a.area, ir.intermediate_region_code, ir.intermediate_region_name, sr.sub_region_name, sr.sub_region_code  
...
from africa_fs_after_cleaning_db a, m49_codes_expanded ir, m49_codes_expanded sr
where ir.m49_code = a.area_code_m49
and sr.m49_code = a.area_code_m49 and a.area = 'Angola';
...
select distinct a.area_code_m49, a.area, --distinct says "don't show repeats"
       ir.intermediate_region_code, ir.intermediate_region_name,
       sr.sub_region_code, sr.sub_region_name 
from africa_fs_after_cleaning_db a, m49_codes_expanded ir, m49_codes_expanded sr
where ir.m49_code = a.area_code_m49
...
-- Q1:  (try this yourself before looking at my answer!)
 ...
 select * from africa_fs_after_cleaning_db af
 where item like ('%Gross%');

-- SANJANA'S ANSWER
 select item_code from africa_fs_after_cleaning_db af
 where item like ('%Gross%');
...
select avg(value), area from africa_fs_after_cleaning_db af
where item_code = '22013' 
group by area_code_m49, area
order by area; --makes the countries listed in alphabetical order
...
select avg(item_code), area, round(avg(value)::numeric, 2) as avg_gdp from africa_fs_after_cleaning_db af
where item_code = '22013' 
group by area_code_m49, area
order by area; --makes the countries listed in alphabetical order
...
select m.intermediate_region_code, m.intermediate_region_name, round(avg(a.value)::numeric, 0) as avg_gdp, max(a.value), min(a.value) --round command will extend to all numeric values
from africa_fs_after_cleaning_db a, m49_codes_expanded m 
where m.m49_code = a.area_code_m49 
and a.item_code = '22013' 
and m.intermediate_region_code != ''
...
select m.intermediate_region_code, m.intermediate_region_name, a.year_code, round(avg(a.value)::numeric, 0) as avg_gdp, max(a.value), min(a.value) --round command will extend to all numeric values
from africa_fs_after_cleaning_db a, m49_codes_expanded m 
where m.m49_code = a.area_code_m49 
and a.item_code = '22013' 
and m.intermediate_region_code != ''
and a.year_code >= 2011 and a.year_code <= 2020
group by m.intermediate_region_code, m.intermediate_region_name, a.year_code 
order by m.intermediate_region_name, a.year_code;
