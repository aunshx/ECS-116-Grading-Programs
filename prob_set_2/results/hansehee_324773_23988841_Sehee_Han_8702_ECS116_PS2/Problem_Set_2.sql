...
-- 
-- the the data looks exactly like the data in the csv file

-- also, as a sanity check, does the count of tuples in your
--    table match the count in the csv file?
-- the count of tuples in my table match the count in the file
...
-- 1) find distinct sub-region names associated with countries in Africa
select distinct sub_region_name 
from m49_codes_expanded mce 
where region_name = 'Africa'
order by sub_region_name ;

-- 2) find intermediate-region names associated with countries in Africa
select distinct intermediate_region_name, country_or_area
from m49_codes_expanded mce 
where region_name = 'Africa'
order by intermediate_region_name , country_or_area;
-- since there are some missing values in intermediate-region names,
-- we also find country or area names instead of that information
...
-- the reason why the result above is coming up empty is
-- that, in the select distinct queries above, the empty values are
--    not NULL, they are ''  (empty string)
-- so the query we want to implement is as below
select m49_code , country_or_area 
from m49_codes_expanded 
where region_name = 'Africa'
...
-- The result above includes not only countries, but also bigger regions
-- Then, how to list only the countries that have
...
select * from africa_fs_ac a where a.area_code_m49 = '024'
...
select count(*)
from africa_fs_ac 
where area_code_m49 = '024'; -- 415 of them
...
order by m.intermediate_region_code;
...
from africa_fs_ac a, m49_codes_expanded m
where m.m49_code = a.area_code_m49 and item_code = '22013'
      and a.year_code::numeric between 2011 and 2020
group by m.intermediate_region_code, m.intermediate_region_name
order by m.intermediate_region_code;
