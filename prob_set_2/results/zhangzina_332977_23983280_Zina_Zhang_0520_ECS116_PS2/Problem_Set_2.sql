...
-- create a query that gives the avg gdp, the max gdp, and the min gdp over the years and 
-- not including countries where intermediate region is empty
select a.year_code, round(avg(value)::numeric, 0) as avg_gdp, round(max(value)::numeric, 0) as max_gdp,
...
select a.year_code, m.intermediate_region_code, m.intermediate_region_name, round(avg(value)::numeric, 0) as avg_gdp, round(max(value)::numeric, 0) as max_gdp,
round(min(value)::numeric, 0) as min_gdp
from africa_fs_after_cleaning_db a, m49_codes_expanded m
...
-- and not including countries where intermediate region is empty
select m.intermediate_region_code, m.intermediate_region_name, round(max(value)::numeric, 0) as max_gdp,
round(min(value)::numeric, 0) as min_gdp
from africa_fs_after_cleaning_db a, m49_codes_expanded m
where a.item_code = '22013'
and m.region_name = 'Africa'
