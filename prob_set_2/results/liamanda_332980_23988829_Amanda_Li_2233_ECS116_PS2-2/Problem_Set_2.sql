...
-- Why are there null values? Some countries don't have an intermediate region
...
-- avg, max, min gdp for each intermediate region
select m.intermediate_region_code,
       m.intermediate_region_name,
       round(avg(value)::numeric, 0) as avg_gpd,
