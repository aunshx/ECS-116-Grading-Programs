...
from m49_codes_expanded 
where region_name = 'Africa'
  AND (intermediate_region_code = 0 OR intermediate_region_code IS NULL)
order by country_or_area;
     
-- had to fix to this because of error
...
-- first try:
-- type mismatch keeps occuring for sample code...
...
where ir.m49_code = a.area_code_m49 -- here the mismatch
  and sr.m49_code = a.area_code_m49;
 --m49 code is int4
 --area_code_m49 is varchar(3) ... ?????
 -- so i will try like this...
 
 -- this causes intermediate_region_code to be null..
 -- I redownloaded the dataset multiple times there still seems to be problem
 SELECT 
    a.area_code_m49,
    a.area,
...
FROM 
    africa_fs_ac a
JOIN 
    m49_codes_expanded ir ON CAST(ir.m49_code AS VARCHAR(3)) = a.area_code_m49
JOIN 
    m49_codes_expanded sr ON CAST(sr.m49_code AS VARCHAR(3)) = a.area_code_m49
ORDER BY 
    a.area;
...
FROM 
    africa_fs_ac a
JOIN 
    m49_codes_expanded m ON m.m49_code::VARCHAR(3) = a.area_code_m49
WHERE 
    a.item_code = '22013'
    -- error due to conflicting types...
GROUP BY 
    m.intermediate_region_code,
    m.intermediate_region_name
ORDER BY 
...
FROM 
    africa_fs_ac a
JOIN 
    m49_codes_expanded m ON CAST(a.area_code_m49 AS INTEGER) = m.m49_code::INTEGER
WHERE 
    a.item_code = '22013'
    AND m.intermediate_region_code IS NOT NULL
GROUP BY 
    m.intermediate_region_code, m.intermediate_region_name
ORDER BY 
    m.intermediate_region_code;
...
-- if it was asking for regions that have avg_gdp lesser/equal to 4000
-- only western africa fits this description though
...
-- only showing countries that are leq 4000
SELECT 
    m.intermediate_region_code,
    m.intermediate_region_name,
    a.area_code_m49,
