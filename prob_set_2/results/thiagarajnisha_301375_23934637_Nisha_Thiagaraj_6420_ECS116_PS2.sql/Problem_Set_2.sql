-- Nisha Thiagaraj
-- I replaced the given answers to the exercises with my answers
...
where intermediate_region_code isnull and region_name = 'Africa'
...
select country_or_area 
from m49_codes_expanded 
where intermediate_region_code = '' and region_name = 'Africa'
...
select distinct m.country_or_area
from m49_codes_expanded m, africa_fs_ac a
where intermediate_region_code = '' and region_name = 'Africa' and m.m49_code = a.area_code_m49
...
select distinct a.area, a.area_code_m49, m.intermediate_region_name, m.intermediate_region_code,
mce.sub_region_name, mce.sub_region_code 
from m49_codes_expanded m, africa_fs_ac a, m49_codes_expanded mce 
where m.m49_code = a.area_code_m49 and mce.m49_code = a.area_code_m49;
...
select area_code_m49, area, round(avg(value)::numeric, 2) as gdp_average
from africa_fs_ac a
where item_code = '22013'
group by area_code_m49, area
...
select area_code_m49, area, to_char(avg(value), 'FM999999999.00') as gdp_average
from africa_fs_ac a
where item_code = '22013'
group by area_code_m49, area
...
select m.intermediate_region_code, m.intermediate_region_name, round(avg(value)::numeric, 0) as gdp_average
from m49_codes_expanded m, africa_fs_ac a
where item_code = '22013' and a.area_code_m49 = m.m49_code
...
select m.sub_region_name , m.sub_region_code , round(avg(value)::numeric, 0) as gdp_average
from m49_codes_expanded m, africa_fs_ac a
where item_code = '22013' and a.area_code_m49 = m.m49_code
...
select m.intermediate_region_code, m.intermediate_region_name,round(avg(value)::numeric, 0) as gdp_average,
round(max(value)::numeric, 0) as gdp_max, round(min(value)::numeric, 0) as gdp_min
from m49_codes_expanded m, africa_fs_ac a
where item_code = '22013' and a.area_code_m49 = m.m49_code and (a.year like '201%' or year like '2020') 
and not (a.year like '%2010%')
group by m.intermediate_region_code, m.intermediate_region_name
...
round(max(value)::numeric, 0) as gdp_max, round(min(value)::numeric, 0) as gdp_min
from m49_codes_expanded m, africa_fs_ac a
where item_code = '22013' and a.area_code_m49 = m.m49_code and where 
group by m.intermediate_region_code, m.intermediate_region_name
